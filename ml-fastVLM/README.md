...
For fast experimentation, the `local_ui/app.py` script wires FastVLM checkpoints into a lightweight Gradio UI.
It captures the latest frame from an RTSP source and lets you prompt the model directly in your browser.

1. Install the dependencies with `pip install -e .` (this pulls in `gradio`, `opencv-python-headless`, and the rest of the stack).
2. Place the FastVLM checkpoint(s) you want to use under `checkpoints/` (e.g., `checkpoints/fastvlm_0.5b_stage3`).
3. Run `python local_ui/app.py` and follow the on-screen instructions; the default RTSP feed is the one you provided.

You can swap checkpoints, adjust sampling hyperparameters, and inject any prompt you like.
